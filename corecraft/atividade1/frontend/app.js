...
async function refreshMempoolInt() {
  const mempoolIntEl = document.getElementById("mempoolIntState");
  const errEl = document.getElementById("mempoolIntError");
  ...
  try {
    ...
    setKV(mempoolIntEl, {
      total_transactions: data.mempool.total_transactions,
      avg_fee_by_bytes: data.mempool.mempool_avg_fee_by_bytes,
      avg_fee_by_tx_count: data.mempool.mempool_avg_fee_by_tx_count,
      low_fee_tx_count: data.mempool_txs.fee_distribution.low_fee,
      medium_fee_tx_count: data.mempool_txs.fee_distribution.medium_fee,
      high_fee_tx_count: data.mempool_txs.fee_distribution.high_fee
    });
...
async function refreshNodeSync() {
  const nodesyncEL = document.getElementById("nodesyncState");
  const errEl = document.getElementById("nodesyncError");
...
document.getElementById("btnRefresh").addEventListener("click", refreshMempoolInt);
document.getElementById("btnRefresh").addEventListener("click", refreshNodeSync);
